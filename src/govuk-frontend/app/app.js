...
module.exports = options => {
  ...
  app.get('/components/all', function(req, res, next) {
    ...
    res.locals.componentData = components.map(componentName => {
      ...
      const defaultExample = componentData.examples.find(example => example.name === 'default');
      return {
        componentName,
  ...
  app.get('/components/:component/:example*?/preview', function(req, res, next) {
    ...
    const exampleConfig = res.locals.componentData.examples.find(example => example.name.replace(/ /g, '-') === requestedExampleName);
